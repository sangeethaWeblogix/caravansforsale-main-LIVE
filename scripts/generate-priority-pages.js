...
### ✅ **1. Image Prefetch (Lines 88-120)** - Already included
- DNS prefetch for `caravansforsale.imagestack.net`
- Preconnect for faster SSL handshake
- Preload first 6 images with `fetchpriority="high"`

### ✅ **2. Routes Mapping Merge (Lines 240-285)** - NEW
- **Fetches existing** routes-mapping from KV before updating
- **Merges** new homepage/listings entries with existing sitemap entries
- **Preserves** all sitemap paths when regenerating priority pages
- Shows **total paths** in final mapping

## What This Fixes:

Before:
```
Priority script runs → Overwrites mapping → Only has "/" and "/listings/"
Sitemap script runs → Overwrites mapping → Only has sitemap paths
```

After:
```
Priority script runs → Merges with existing → Has "/" + "/listings/" + all sitemap paths
Sitemap script runs → Merges with existing → Has "/" + "/listings/" + all sitemap paths
```

## Deploy & Test:

1. **Replace** your current `scripts/generate-priority-pages.js` with this version
2. **Commit and push** to GitHub
3. **Run workflow**: GitHub Actions → "Generate Cache" → "priority"
4. **Check output** - you'll see:
```
   ✅ Loaded existing mapping with 70 paths
   📊 Total paths in mapping: 72
