...
//  const [firstSix, setFirstSix] = useState<string[]>([]);
...
//       if (!cancelled) {
//         setFirstSix(fallback.slice(1, 6));
//         setRemaining(fallback.slice(6));
...
//     if (await checkImage(mainUrl)) urls.push(mainUrl);

//     // subs (load as many as exist)
//     for (let i = 1; i <= 50; i++) {
...
//     const final = urls.length ? urls : (images.length ? images : [main]);

//     if (!cancelled) {
//       setFirstSix(final.slice(0, 6));
//       setRemaining(final.slice(6));
//       setActiveImage(final[0] || main);
//     }
//   };
...
//         {firstSix.slice(0, 4).map((image, i) => (
