...
export default function ListingsPage({
  ...
  // );
  // ⬅️ Add this ref at the top of your component
  const debounceTimerRef = useRef<NodeJS.Timeout | null>(null);

  const updateURLWithFilters = useCallback(
    (nextFilters: Filters, pageNum: number) => {
      // ✅ clear old timer if user clicks/changes fast
      ...
      debounceTimerRef.current = setTimeout(() => {
        ...
        if (nextFilters.orderby)
          query.set("orderby", String(nextFilters.orderby));
        if (nextFilters.acustom_fromyears)
          query.set("acustom_fromyears", String(nextFilters.acustom_fromyears));
        if (nextFilters.acustom_toyears)
          query.set("acustom_toyears", String(nextFilters.acustom_toyears));
        ...
        router.push(finalURL); // ✅ only fires once after debounce
      }, 400); // ⏱ debounce time in ms (tweak if needed)
    },
    [router, DEFAULT_RADIUS]
  );
  ...
  const handleFilterChange = useCallback(
    (newFilters: Filters) => {
      ...
      // ✅ reset pagination (always go to page 1 on filter change)
      setPagination({
        current_page: 1,
        total_pages: 1,
        total_items: 0,
      ...
      // ✅ update URL (this will trigger your useEffect watcher → API call)
      updateURLWithFilters(mergedFilters, 1);
    },
