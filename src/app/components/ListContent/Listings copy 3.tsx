   ...
   import dynamic from 'next/dynamic';
    ...
    { 
      ssr: false,
      loading: () => <div className="skeleton-placeholder">Loading...</div>
    }
  );
  ...
  router.replace(finalURL, { scroll: false });   setTimeout(() => {
  ...
  // Update the isHydrated initialization (around line 235)
  const [isHydrated, setIsHydrated] = useState(() => {
    // Only consider hydrated if we have actual product data
    return !!(initialData?.data?.products?.length);
  });
  const hasClientFetchedRef = useRef(false);
 
  useEffect(() => {
    if (!initialData) return;
    if (hasClientFetchedRef.current) return;
 
    if (initialData.data?.products) {
      setProducts(transformApiItemsToProducts(initialData.data.products));
    ...
    setIsHydrated(true);
     ...
     fetchInProgressRef.current = true;
         hasClientFetchedRef.current = true;
      ...
      flushSync(() => {
       ...
       setIsPremiumLoading(true);
         ...
         try {
           ...
           setPageTitle(response?.list_page_title ?? "");  // ✅ ADD THIS LINE
   ...
   const handlePrevPage = useCallback(async () => {
     ...
     useEffect(() => {
       if (!initializedRef.current) return;
    if (clearAllInProgressRef.current) return;
  
    // 🚫 skip once after clear-all
   
    if (skipNextUrlEffectRef.current) {
      console.log("Skipping URL effect - manual update in progress");
      return;
    }
   ...
   await loadListings(1, mergedFilters, true);  // 👈 response capture பண்ணுங்க
        
        // 👇 Explicitly set pageTitle from response
   ...
   const clearAllInProgressRef = useRef(false);
  
  const resetAllFilters = async () => {
    if (!hasActiveFilters) return;
  
    // 🔒 lock everything
    clearAllInProgressRef.current = true;
    isClearAllRef.current = true;
  
    // 🔥 IMMEDIATELY show skeleton (prevents white screen)
    flushSync(() => {
      setIsLoading(true);
      setIsMainLoading(true);
      setIsFeaturedLoading(true);
    ...
    try {
      // 🔹 clear URL (NO FETCH HERE)
      router.replace("/listings", { scroll: false });
  
      // 🔹 force default fetch
      await loadListings(1, clearedFilters, true);
  
    } catch (err) {
      console.error("Clear all failed:", err);
    } finally {
      // 🔓 unlock
      clearAllInProgressRef.current = false;
   ...
   isPremiumLoading ? (
                  ...
                  {/* 4️⃣ SAFETY FALLBACK – NEVER WHITE */}
   {products.length === 0 &&
        fetauredProducts.length === 0 &&
        preminumProducts.length === 0 &&
        emptyProduct.length === 0 && (
