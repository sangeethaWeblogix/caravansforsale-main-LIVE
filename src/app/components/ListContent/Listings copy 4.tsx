 ...
 type ErrorType = "network" | "api" | "empty" | null;
 ...
 function isNetworkError(error: unknown): boolean {
   ...
   // Check for fetch AbortError or network-related DOMException
 ...
 export default function ListingsPage({
   ...
   const [errorState, setErrorState] = useState<ErrorType>(null);
 ...
 useEffect(() => {
  ...
  const updateURLWithFilters = useCallback((nextFilters: Filters, pageNum: number) => {
   ...
   const loadListings = useCallback(
     async (
       ...
       setErrorState(null);
       setErrorMessage("");
       setIsLoading(true);  // 👈 இங்கே add பண்ணு
     });
  ...
  if (response && response.success === false) {
           ...
           setErrorState("api");
           setErrorMessage(
             response.message || "Sever error occurred. We're working on it!"
           );
           setProducts([]);
           return undefined;
         }
         ...
         const productsList = response?.data?.products ?? [];
           ...
           products.some(
             (p) => p && Object.keys(p).length > 0 && (p.id || p.slug)
           );
         ...
         if (response.pagination) setPagination(response.pagination);
           ...
           setErrorState("empty");
           setErrorMessage(
             "No caravans found matching your filters. Try adjusting your search criteria."
           );
           setProducts([]);
         }
         ...
         // ✅ Detect error type: Network vs API
         if (isNetworkError(error)) {
           ...
           setErrorState("api");
           setErrorMessage(
             "Something went wrong loading listings. We're working on it!"
           );
         }
 ...
 const resetAllFilters = useCallback(async () => {
   // Remove hasActiveFilters check - always allow clear
   isClearAllRef.current = true;
 
   // ✅ FIRST: Set loading states synchronously
   flushSync(() => {
     setIsLoading(true);
     setIsMainLoading(true);
     setIsFeaturedLoading(true);
   ...
   try {
     ...
     // ✅ IMPORTANT: Call loadListings to fetch fresh data
                 ...
                 <div className="filter filter_sticky hidden-xs hidden-sm">
                   <div className="card-title align-items-center d-flex justify-content-between hidden-xs">
                     ...
                     <span className="text-uppercase clear_btn">
                        {/* ✅ Complete hydration-safe button */}
 <button
   onClick={resetAllFilters}
   disabled={!hasActiveFilters || (isMounted && (isLoading || isMainLoading))}
   className="clear_btn"
   style={{ 
     border: "none", 
     backgroundColor: "white",
     cursor: (!hasActiveFilters || isLoading) ? "not-allowed" : "pointer",
     opacity: (!hasActiveFilters || isLoading) ? 0.6 : 1
   }}
 >
   {isMounted && isLoading ? (
     <><i className="bi bi-hourglass-split me-1"></i> Clearing...</>
   ) : (
  ...
  isPremiumLoading ? (
   ...
   // ✅ Error state - show appropriate error message (CENTERED)
   <ApiErrorFallback
     errorType={errorState}
     onClearFilters={
       errorState === "empty" ? handleClearFilters : undefined
     }
   />
 ) : (
   <>
