...
const CaravanFilter: React.FC<CaravanFilterProps> = ({
  ...
  useEffect(() => {
    console.log(
      "📍 REGION STATE UPDATED - selectedRegionName:",
      selectedRegionName
    );
  }, [selectedRegionName]);
  ...
  useEffect(() => {
    // Only run if we have suburb but NO region (from manual dropdown selection)
    if (
      ...
      states.length === 0 || // No states data
      suburbClickedRef.current // Manual selection in progress
    ) {
      return;
    }

    console.log(
      "🧭 Auto-detecting region for manually selected suburb:",
      selectedSuburbName
    );
    ...
    // Find region that contains this suburb
    ...
    if (matchedRegion) {
      console.log("✅ Auto-detected region from dropdown:", matchedRegion.name);
      ...
      console.log(
        "❌ No region found in static data for suburb:",
        selectedSuburbName
      );
    }
  ...
  // Add this useEffect to maintain the correct panel state
  useEffect(() => {
                        ...
                        return (
                          <li
                            ...
                            onMouseDown={(e) => {
                              ...
                              console.log("🎯 Search selection:", item);

                              // Extract location details from the suggestion
                              ...
                              if (!/^\d{4}$/.test(pincode)) {
                                const m = item.address.match(/\b\d{4}\b/);
                                if (m) pincode = m[0];
                              }
                              ...
                              // ✅ Set ALL location states immediately
                              setSelectedState(stateSlug);
                              setSelectedStateName(AUS_ABBR[state] || state);
                              ...
                              // ✅ Update filters with ALL location data
                              ...
                              console.log(
                                "✅ Search selection complete - Region set:",
                                region
                              );
                            }}
                          >
